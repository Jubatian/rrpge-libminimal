...
void rrpge_m_grln(void)
{
 ...
 dlin = &(rrpge_m_edat->vram[(i & (~(auint)(3U))) << 9]);
 dsiz = (i & 3U) + 2U;
 ...
 if (dbl != 0U){ /* Double scan */
  dsiz ++;
  cyr = 392U;
  if ((rrpge_m_info.vln & 1U) != 0U){ return; } /* Odd lines have no render */
 }
 dsiz = (auint)(1U) << dsiz;
 ...
 ** display list, or exhausting the cycle budget. */
 ...
 while (1){
  ...
  if ((cmd & 0xBC00U) != 0U){    /* Render command not inactive */
   ...
   /* Source line select in cmd is no longer used. Substitute with the current
   ** graphics mode for accessing the latter faster later on */
